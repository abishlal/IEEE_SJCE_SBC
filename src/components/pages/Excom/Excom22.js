import React from "react";
import './excom.css';

const Excom22 = () => {
  return (
    <div className="body">
      <div className="sbc">
        <div class="nine">
          <h1>Student Branch Office Bearers<span>Excom 2022</span></h1>
        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Sreekanth R
              </h2>
            </div>
            <div className="details">
              <h2>Sreekanth R
                <span>COUNSELLOR
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Mohamed Aashik S
              </h2>
            </div>
            <div className="details">
              <h2>Mohamed Aashik S
                <span>CHAIR PERSON
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Arisudan T
              </h2>
            </div>
            <div className="details">
              <h2>Arisudan T
                <span>VICE CHAIR
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>


        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Priyanka J
              </h2>
            </div>
            <div className="details">
              <h2>Priyanka J
                <span>SECRETARY
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Ranjani S
              </h2>
            </div>
            <div className="details">
              <h2>Ranjani S
                <span>JOINT SECRETARY
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Karthik Kumar C
              </h2>
            </div>
            <div className="details">
              <h2>Karthik Kumar C
                <span>TREASURER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Abishlal N S
              </h2>
            </div>
            <div className="details">
              <h2>Abishlal N S
                <span>WEBMASTER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>

        </div>
      </div>
      <div className="ias">
        <div class="nine">
          <h1>Industrial Applications Society Office Bearers<span>Excom 2022</span></h1>
        </div>

        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Dr.Jayarama Pradeep
              </h2>
            </div>
            <div className="details">
              <h2>Dr.Jayarama Pradeep
                <span>ADVISOR
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Denora S
              </h2>
            </div>
            <div className="details">
              <h2>Denora S
                <span>CHAIR PERSON
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Kavipriya D
              </h2>
            </div>
            <div className="details">
              <h2>Kavipriya D
                <span>VICE CHAIR
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Ananya P K
              </h2>
            </div>
            <div className="details">
              <h2>Ananya P K
                <span>SECRETARY
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Karthikeyan S
              </h2>
            </div>
            <div className="details">
              <h2>Karthikeyan S
                <span>JOINT SECRETARY
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Prasanna Gnanambiga S
              </h2>
            </div>
            <div className="details">
              <h2>Prasanna Gnanambiga S
                <span>TREASURER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Hema Apsara A
              </h2>
            </div>
            <div className="details">
              <h2>Hema Apsara A
                <span>COMMITTEE MEMBER - 1
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Rajesh S
              </h2>
            </div>
            <div className="details">
              <h2>Rajesh S
                <span>COMMITTEE MEMBER - 2
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>



        </div>
      </div>
      <div className="ras">
        <div class="nine">
          <h1>Robotics and Automation Society Office Bearers<span>Excom 2022</span></h1>
        </div>

        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Sreekanth R
              </h2>
            </div>
            <div className="details">
              <h2>Sreekanth R
                <span>COUNSELLOR
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Abina S
              </h2>
            </div>
            <div className="details">
              <h2>Abina S
                <span>CHAIR PERSON
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Aparnaa G
              </h2>
            </div>
            <div className="details">
              <h2>Aparnaa G
                <span>VICE CHAIR
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Abinaya M
              </h2>
            </div>
            <div className="details">
              <h2>Abinaya M
                <span>SECRETARY
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Keerthana S
              </h2>
            </div>
            <div className="details">
              <h2>Keerthana S
                <span>JOINT SECRETARY
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Sunitha K
              </h2>
            </div>
            <div className="details">
              <h2>Sunitha K
                <span>TREASURER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Nithin Joel J
              </h2>
            </div>
            <div className="details">
              <h2>Nithin Joel J
                <span>COMMITTEE MEMBER - 1
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Gokul Kumar M
              </h2>
            </div>
            <div className="details">
              <h2>Gokul Kumar M
                <span>COMMITTEE MEMBER - 2
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>



        </div>
      </div>
      <div className="pels">
        <div class="nine">
          <h1>Power Electronics Society Office Bearers<span>Excom 2022</span></h1>
        </div>

        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Sreekanth R
              </h2>
            </div>
            <div className="details">
              <h2>Sreekanth R
                <span>COUNSELLOR
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Elanchezhiyan R J
              </h2>
            </div>
            <div className="details">
              <h2>Elanchezhiyan R J
                <span>CHAIR PERSON
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Karthikeyan M
              </h2>
            </div>
            <div className="details">
              <h2>Karthikeyan M
                <span>VICE CHAIR
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Jesse S
              </h2>
            </div>
            <div className="details">
              <h2>Jesse S
                <span>SECRETARY
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Alwyn Samuel M
              </h2>
            </div>
            <div className="details">
              <h2>Alwyn Samuel M
                <span>JOINT SECRETARY
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Divya Lakshmi G
              </h2>
            </div>
            <div className="details">
              <h2>Divya Lakshmi G
                <span>TREASURER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Poojasree K
              </h2>
            </div>
            <div className="details">
              <h2>Poojasree K
                <span>COMMITTEE MEMBER - 1
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Hemaa K
              </h2>
            </div>
            <div className="details">
              <h2>Hemaa K
                <span>COMMITTEE MEMBER - 2
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>


        </div>
      </div>
      <div className="wie">
        <div class="nine">
          <h1>Women In Engineering Affinity Group Office Bearers<span>Excom 2022</span></h1>
        </div>

        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Dr.Jayarama Pradeep
              </h2>
            </div>
            <div className="details">
              <h2>Dr.Jayarama Pradeep
                <span>ADVISOR
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Abinayaa Sri T
              </h2>
            </div>
            <div className="details">
              <h2>Abinayaa Sri T
                <span>CHAIR PERSON
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Jamuna U
              </h2>
            </div>
            <div className="details">
              <h2>Jamuna U
                <span>VICE CHAIR
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Christel Peeris R
              </h2>
            </div>
            <div className="details">
              <h2>Christel Peeris R
                <span>SECRETARY
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Kaviya P
              </h2>
            </div>
            <div className="details">
              <h2>Kaviya P
                <span>JOINT SECRETARY
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Ramya R
              </h2>
            </div>
            <div className="details">
              <h2>Ramya R
                <span>TREASURER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Deepika U
              </h2>
            </div>
            <div className="details">
              <h2>Deepika U
                <span>COMMITTEE MEMBER - 1
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Dharani V
              </h2>
            </div>
            <div className="details">
              <h2>Dharani V
                <span>COMMITTEE MEMBER - 2
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>


        </div>
      </div>
      <div className="pc">
        <div class="nine">
          <h1>PROGRAM COORDINATOR OF IEEE SJCE SB​<span>Excom 2022</span></h1>
        </div>

        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Swetha B
              </h2>
            </div>
            <div className="details">
              <h2>Swetha B
                <span>PROGRAM COORDINATOR
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>

        </div>
      </div>
      <div className="conference">
        <div class="nine">
          <h1>CONFERENCE TEAM​<span>Excom 2022</span></h1>
        </div>

        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Thiruvel A P
              </h2>
            </div>
            <div className="details">
              <h2>Thiruvel A P
                <span>LEAD
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Rajesh S
              </h2>
            </div>
            <div className="details">
              <h2>Rajesh S
                <span>CO-LEAD
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Bertina S
              </h2>
            </div>
            <div className="details">
              <h2>Bertina S
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>ShivKumar Eswar B
              </h2>
            </div>
            <div className="details">
              <h2>ShivKumar Eswar B
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Yavanesh J
              </h2>
            </div>
            <div className="details">
              <h2>Yavanesh J
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Narendra S
              </h2>
            </div>
            <div className="details">
              <h2>Narendra S
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
      </div>
      <div className="goodiesteam">
        <div class="nine">
          <h1>AWARDS AND GOODIES TEAM​<span>Excom 2022</span></h1>
        </div>

        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Jesse S
              </h2>
            </div>
            <div className="details">
              <h2>Jesse S
                <span>LEAD
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Jayavarshini V
              </h2>
            </div>
            <div className="details">
              <h2>Jayavarshini V
                <span>CO-LEAD
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Shahin R J
              </h2>
            </div>
            <div className="details">
              <h2>Shahin R J
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Joel Ebinezar J
              </h2>
            </div>
            <div className="details">
              <h2>Joel Ebinezar J
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Manigandan V
              </h2>
            </div>
            <div className="details">
              <h2>Manigandan V
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Naveen Buyavarapu
              </h2>
            </div>
            <div className="details">
              <h2>Naveen Buyavarapu
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
      </div>
      <div className="designteam">
        <div class="nine">
          <h1>GRAPHIC DESIGN TEAM​<span>Excom 2022</span></h1>
        </div>

        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Kavin D
              </h2>
            </div>
            <div className="details">
              <h2>Kavin D
                <span>LEAD
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Abiksitha R
              </h2>
            </div>
            <div className="details">
              <h2>Abiksitha R
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Hariharan N
              </h2>
            </div>
            <div className="details">
              <h2>Hariharan N
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Sumanth K
              </h2>
            </div>
            <div className="details">
              <h2>Sumanth K
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Shyfullah P M
              </h2>
            </div>
            <div className="details">
              <h2>Shyfullah P M
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Eureak Singh S
              </h2>
            </div>
            <div className="details">
              <h2>Eureak Singh S
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
      </div>
      <div className="contentteam">
        <div class="nine">
          <h1>CONTENT CREATION TEAM​<span>Excom 2022</span></h1>
        </div>

        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Maria Shirley John L M
              </h2>
            </div>
            <div className="details">
              <h2>Maria Shirley John L M
                <span>LEAD
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Poornavikash T
              </h2>
            </div>
            <div className="details">
              <h2>Poornavikash T
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Darsini R
              </h2>
            </div>
            <div className="details">
              <h2>Darsini R
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Rathna Devi S
              </h2>
            </div>
            <div className="details">
              <h2>Rathna Devi S
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Praveen Kumar J
              </h2>
            </div>
            <div className="details">
              <h2>Praveen Kumar J
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Chureender T
              </h2>
            </div>
            <div className="details">
              <h2>Chureender T
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
      </div>
      <div className="publicityteam">
        <div class="nine">
          <h1>PUBLICITY TEAM<span>Excom 2022</span></h1>
        </div>

        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Prasanth Kumar A V
              </h2>
            </div>
            <div className="details">
              <h2>Prasanth Kumar A V
                <span>LEAD
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Thavaloshini A
              </h2>
            </div>
            <div className="details">
              <h2>Thavaloshini A
                <span>CO-LEAD
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Nandi Vardhan Reddy R
              </h2>
            </div>
            <div className="details">
              <h2>Nandi Vardhan Reddy R
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Shankar Aahash S
              </h2>
            </div>
            <div className="details">
              <h2>Shankar Aahash S
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Haresh R
              </h2>
            </div>
            <div className="details">
              <h2>Haresh R
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>

        </div>
      </div>
      <div className="colobteam">
        <div class="nine">
          <h1>COLLOBORATION TEAM<span>Excom 2022</span></h1>
        </div>

        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Udhayaraga B
              </h2>
            </div>
            <div className="details">
              <h2>Udhayaraga B
                <span>LEAD
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Evangelene M
              </h2>
            </div>
            <div className="details">
              <h2>Evangelene M
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Prasanna Gnanambiga S
              </h2>
            </div>
            <div className="details">
              <h2>Prasanna Gnanambiga S
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Prema D
              </h2>
            </div>
            <div className="details">
              <h2>Prema D
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Faizaan Ahmed Y
              </h2>
            </div>
            <div className="details">
              <h2>Faizaan Ahmed Y
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Alwyn Samuel M
              </h2>
            </div>
            <div className="details">
              <h2>Alwyn Samuel M
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
      </div>
      <div className="webteam">
        <div class="nine">
          <h1>WEBSITE TEAM<span>Excom 2022</span></h1>
        </div>

        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Abishlal N S
              </h2>
            </div>
            <div className="details">
              <h2>Abishlal N S
                <span>LEAD
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Mugundhan B
              </h2>
            </div>
            <div className="details">
              <h2>Mugundhan B
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Linda Joice R P
              </h2>
            </div>
            <div className="details">
              <h2>Linda Joice R P
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Abinaya Priya R A
              </h2>
            </div>
            <div className="details">
              <h2>Abinaya Priya R A
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Aruna Devi S
              </h2>
            </div>
            <div className="details">
              <h2>Aruna Devi S
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Karthikeyan M
              </h2>
            </div>
            <div className="details">
              <h2>Karthikeyan M
                <span>COMMITTEE MEMBER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
      </div>
      <div className="specialdayteam">
        <div class="nine">
          <h1>SPECIAL DAY TEAM<span>Excom 2022</span></h1>
        </div>

        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Jenin Benedict A
              </h2>
            </div>
            <div className="details">
              <h2>Jenin Benedict A
                <span>LEAD
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Hema Apsara A
              </h2>
            </div>
            <div className="details">
              <h2>Hema Apsara A
                <span>CO-LEAD
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>


        </div>
      </div>
      <div className="tamil">
        <div class="nine">
          <h1>TAMIL MANDRAM<span>Excom 2022</span></h1>
        </div>

        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Mani Sha N
              </h2>
            </div>
            <div className="details">
              <h2>Mani Sha N
                <span>CHAIR PERSON
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Jayashruthi R V
              </h2>
            </div>
            <div className="details">
              <h2>Jayashruthi R V
                <span>VICE CHAIR
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Akshaya Krishnen K
              </h2>
            </div>
            <div className="details">
              <h2>Akshaya Krishnen K
                <span>SECRETARY
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>




        </div>
        <div className="box">
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Amirtha Varshini M
              </h2>
            </div>
            <div className="details">
              <h2>Amirtha Varshini M
                <span>JOINT SECRETARY
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>
          <div className="excomCard">
            <div className="rot">
              <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500">
                <title>Desination</title>
                <defs>
                  <path d="M50,250c0-110.5,89.5-200,200-200s200,89.5,200,200s-89.5,200-200,200S50,360.5,50,250" id="textcircle">
                    <animateTransform
                      attributeName="transform"
                      begin="0s"
                      dur="30s"
                      type="rotate"
                      from="0 250 250"
                      to="360 250 250"
                      repeatCount="indefinite"
                    />
                  </path>

                </defs>

                <use xlinkHref="#redstripe" transform="rotate(90 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(45 250 250)" />
                <use xlinkHref="#redstripe" transform="rotate(-45 250 250)" />
                <text dy="70" textLength="1220">

                  <textPath xlinkHref="#textcircle">EXECOM 2022 * EXECOM 2022 *</textPath>
                </text>
              </svg>
            </div>
            <div className="imgBx">
              <img src="https://images.unsplash.com/photo-1532123675048-773bd75df1b4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=60" alt="images"></img>
            </div>
            <div className="detail">
              <h2>Godson S V Noble
              </h2>
            </div>
            <div className="details">
              <h2>Godson S V Noble
                <span>TREASURER
                </span></h2>
              <a href="#" class="fa fa-linkedin"></a>
              <a href="#" class="fa fa-envelope"></a>
            </div>
          </div>


        </div>
      </div>
      <div className="com society">
        <div class="nine">
          <h1>Communications Society <span>Execom 2022</span></h1>
        </div>
        <table>
          <tr>
            <th>NAME OF STUDENT</th>
            <th>POST ASSIGNED</th>
          </tr>
          <tr>
            <td>VIGNESH D </td>
            <td>CHAIRMAN</td>
          </tr>
          <tr>
            <td>SRIMATHI J</td>
            <td>SECRETARY</td>
          </tr>
          <tr>
            <td>SRINIVEDHINIV V</td>
            <td>TREASURER</td>
          </tr>
        </table>
      </div>
      <div className="Broadcast society">
        <div class="nine">
          <h1>Broadcast Technology Society<span>Execom 2022</span></h1>
        </div>
        <table>
          <tr>
            <th>NAME OF STUDENT</th>
            <th>POST ASSIGNED</th>
          </tr>
          <tr>
            <td>VADDI LAKSHMI SATHYA SAI SAROJINI </td>
            <td>CHAIRMAN</td>
          </tr>
          <tr>
            <td>PREETHI G</td>
            <td>SECRETARY</td>
          </tr>
          <tr>
            <td>SUSHANTH KRISHNAMITHRAN</td>
            <td>TREASURER</td>
          </tr>
        </table>
      </div>
      <div className="Industrial Electronics society">
        <div class="nine">
          <h1>Industrial Electronics Society<span>Execom 2022</span></h1>
        </div>
        <table>
          <tr>
            <th>NAME OF STUDENT</th>
            <th>POST ASSIGNED</th>
          </tr>
          <tr>
            <td>JAYASHREE G</td>
            <td>CHAIRMAN</td>
          </tr>
          <tr>
            <td>RAJAGOPALAN V G</td>
            <td>SECRETARY</td>
          </tr>
          <tr>
            <td>SWEETY R</td>
            <td>TREASURER</td>
          </tr>
        </table>
      </div>
      <div className="Electron Devices Society">
        <div class="nine">
          <h1>Electron Devices Society<span>Execom 2022</span></h1>
        </div>
        <table>
          <tr>
            <th>NAME OF STUDENT</th>
            <th>POST ASSIGNED</th>
          </tr>
          <tr>
            <td>RHAGUL D</td>
            <td>CHAIRMAN</td>
          </tr>
          <tr>
            <td>LEE MANOJ DARSHAN N</td>
            <td>SECRETARY</td>
          </tr>
          <tr>
            <td>AMALWIN ASEER S</td>
            <td>TREASURER</td>
          </tr>
        </table>
      </div>
      <div className="Antenna Propagation Society">
        <div class="nine">
          <h1>Antenna Propagation Society<span>Execom 2022</span></h1>
        </div>
        <table>
          <tr>
            <th>NAME OF STUDENT</th>
            <th>POST ASSIGNED</th>
          </tr>
          <tr>
            <td>MATHI VADHANA P</td>
            <td>CHAIRMAN</td>
          </tr>
          <tr>
            <td>RAMA KRISHNAN H</td>
            <td>SECRETARY</td>
          </tr>
          <tr>
            <td>RANJITH ROSHAN R</td>
            <td>TREASURER</td>
          </tr>
        </table>
      </div>
      <div className="Engineering Medical and Biology Society ">
        <div class="nine">
          <h1>Engineering Medical and Biology Society<span>Execom 2022</span></h1>
        </div>
        <table>
          <tr>
            <th>NAME OF STUDENT</th>
            <th>POST ASSIGNED</th>
          </tr>
          <tr>
            <td>NAVEENA R</td>
            <td>CHAIRMAN</td>
          </tr>
          <tr>
            <td>KEERTHANA R M</td>
            <td>SECRETARY</td>
          </tr>
          <tr>
            <td>RACHEL A</td>
            <td>TREASURER</td>
          </tr>
        </table>
      </div>
    </div>
  );
};

export default Excom22;



